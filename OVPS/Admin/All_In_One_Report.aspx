<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/MasterPageUser.master" AutoEventWireup="true" CodeFile="All_In_One_Report.aspx.cs" Inherits="All_In_One_Report" %>
...
<StaticSelectedStyle CssClass="selectedTab" BackColor="#009999"></StaticSelectedStyle>
    ...
    <asp:MultiView  
        ...
        <div style="overflow:scroll; width:750px; height:600px;">
                 <% if (objDs_bank.Tables.Count > 0)
               {
                   if (objDs_bank.Tables[0].Rows.Count > 0)
                   { %>
                   <div style="overflow:scroll;">
                    ...
                    <%
                       for (int k = 0; k < objDs_bank.Tables[0].Rows.Count; k++)
                       { 
                       %>
                       <tr>
                             <th align="center" class="b11" style="white-space: nowrap;" valign="middle"><%= Convert.ToString(objDs_bank.Tables[0].Rows[k]["Month_S"]) +" " + Convert.ToString(objDs_prod.Tables[0].Rows[k]["Year_S"]) %></th>
                              <%
                       for (int i = 0; i < objDs_prod.Tables[0].Rows.Count; i++)
                       {
                           if (Convert.ToString(objDs_bank.Tables[0].Rows[k]["Year_S"]) == Convert.ToString(objDs_prod.Tables[0].Rows[i]["Year_S"]))
                              {
                       %>
                                <th align="center" class="b11" style="white-space: nowrap;" valign="middle"><%= Convert.ToString(objDs_prod.Tables[0].Rows[k]["ZoneName"]) %></th>                                
                             
                                <th align="center" class="b11" style="white-space: nowrap;" valign="middle"><%= Convert.ToString(objDs_bank.Tables[0].Rows[k]["AO_S"]) %></th>                                
                                <th align="center" class="b11" style="white-space: nowrap;" valign="middle"><%= Convert.ToString(objDs_bank.Tables[0].Rows[k]["CR_S"]) %></th>
                         ...
                         <%
                           ...
                           for (int j = 0; j < objDs_wasted.Tables[0].Rows.Count; j++)
                           {
                               ...
                               //if (Convert.ToString(objDs_prod.Tables[0].Rows[i]["Month_S"]) + " " + Convert.ToString(objDs_prod.Tables[0].Rows[i]["Year_S"]) == Convert.ToString(objDs_wasted.Tables[0].Rows[j]["Month"]) + " " + Convert.ToString(objDs_wasted.Tables[0].Rows[j]["Year"]) )
                               if (Convert.ToString(Convert.ToString(objDs_prod.Tables[0].Rows[i]["Year_S"])) ==  Convert.ToString(objDs_wasted.Tables[0].Rows[j]["Year"]) )
                               {
                                   flg = 1;
