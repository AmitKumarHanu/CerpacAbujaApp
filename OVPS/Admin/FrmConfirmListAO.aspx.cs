...
public partial class Admin_FrmConfirmListAO : System.Web.UI.Page
{
    ...
    protected void Go_Click(object sender, EventArgs e)
    {
   	...
   	string quer = "select a.*,b.* from People as a , PeopleChild as b where a.cerpac_no = b.CerpacNo and a.cerpac_no='" + TextAppId.Text.ToString() + "' and b.isVerified=1 and b.isAuthorized =1 and(b.isARCR = 0 or b.isARCR is null) and (FORMNO Like 'AO%' or FORMNO Like 'CR%' or FORMNO Like 'AB%')";
    ...
    public void getRec(string cer)
    {
        ...
        if (dt.Rows.Count > 0 && (dt.Rows[0]["ISARCR"].ToString().Trim() == "0" || dt.Rows[0]["ISARCR"].ToString().Trim() == "" || dt.Rows[0]["ISARCR"] == null) && dt.Rows[0]["IsAuthorized"].ToString().Trim() == "1" && dt.Rows[0]["IsVerified"].ToString().Trim() == "1")
        {
            Response.Redirect("FrmConfirmEligibilityAO.aspx?no=" + cer + "");
        }
        else if (dt.Rows.Count > 0 && dt.Rows[0]["IsARCR"].ToString() == "1")
        {
            lblmsg.Text = "This Cerpac Number Has already been Authorized";
    ...
    protected void GridViewCntcAuthAO_RowCreated(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            e.Row.Cells[3].Attributes["onclick"] = this.Page.ClientScript.GetPostBackClientHyperlink(this.GridViewCntcAuthAO, "Select$" + e.Row.RowIndex);
            e.Row.Cells[4].Attributes["onclick"] = this.Page.ClientScript.GetPostBackClientHyperlink(this.GridViewCntcAuthAO, "Select$" + e.Row.RowIndex);
