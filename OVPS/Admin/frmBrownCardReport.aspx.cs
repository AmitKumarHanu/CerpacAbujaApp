...
public partial class Admin_frmBrownCardReport : System.Web.UI.Page
{
    #region "Declarations"
    ...
    protected void btn_generate_report_Click(object sender, EventArgs e)
    {
        ...
        if (txtFromDate.Value != "" && txtToDate.Value != "")
        {
            try
            {
                ...
                DataTable dt = new DataTable();
                string query = "";
                query = "SELECT TOP (100) PERCENT ISNULL(dbo.vw_rep_cnt1.Month, dbo.vw_rep_cnt_brown.Month) AS Month_S, ISNULL(dbo.vw_rep_cnt1.Year, dbo.vw_rep_cnt_brown.Year) AS Year_S," +
                        " dbo.vw_rep_cnt1.AO AS AO_S, dbo.vw_rep_cnt1.AB AS AB_S, dbo.vw_rep_cnt1.CR AS CR_S, ISNULL(dbo.vw_rep_cnt_brown.Total, 0) AS Total, ISNULL(dbo.vw_rep_cnt_brown.ZoneName, '') " +
                        " AS ZoneName, ISNULL(dbo.vw_rep_cnt_brown.AO, 0) AS AO,  ISNULL(dbo.vw_rep_cnt_brown.AB, 0) AS AB,  ISNULL(dbo.vw_rep_cnt_brown.AR, 0) AS AR, ISNULL(dbo.vw_rep_cnt_brown.CR, 0) AS CR, dbo.vw_rep_cnt1.mon " +
                        " FROM dbo.vw_rep_cnt_brown FULL OUTER JOIN   dbo.vw_rep_cnt1 ON dbo.vw_rep_cnt_brown.Month = dbo.vw_rep_cnt1.Month AND dbo.vw_rep_cnt_brown.Year = dbo.vw_rep_cnt1.Year " +
                        " where dbo.vw_rep_cnt_brown.Month='" + dpdMon.SelectedItem + "' and ISNULL(dbo.vw_rep_cnt1.Year, dbo.vw_rep_cnt_brown.Year)=" + dpdYear.SelectedValue + "  ORDER BY Year_S, dbo.vw_rep_cnt1.mon";
