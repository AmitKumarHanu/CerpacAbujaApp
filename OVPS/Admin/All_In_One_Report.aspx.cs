...
public partial class All_In_One_Report : System.Web.UI.Page
{
    ...
    protected DataSet objDs_bank = new DataSet();
        ...
        if (!IsPostBack)
        {
          ...
          //  btn_generate_report0.Attributes.Add("onclick", "if(!pwd('Submit')) return false;");
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                Session["ZoneName"] = dt.Rows[0]["zonecode"].ToString();
            }
    ...
    protected void btn_generate_report_Click(object sender, EventArgs e)
    {
        ...
        if (txtFromDate.Value != "" && txtToDate.Value != "")
        {
            try
            {
                ...
                querybank = "SELECT TOP (100) PERCENT (dbo.vw_rep_cnt1.Month) AS Month_S, (dbo.vw_rep_cnt1.Year) AS Year_S,  dbo.vw_rep_cnt1.AO AS AO_S, dbo.vw_rep_cnt1.AB AS AB_S, dbo.vw_rep_cnt1.CR AS CR_S,    dbo.vw_rep_cnt1.mon FROM  dbo.vw_rep_cnt1 where dbo.vw_rep_cnt1.Month='" + dpdMon.SelectedItem + "' and (dbo.vw_rep_cnt1.Year)=" + dpdYear.SelectedValue + "  ORDER BY Year_S, dbo.vw_rep_cnt1.mon";
                dt = objgenenral.FetchData(querybank);
                objDs_bank = SqlHelper.ExecuteDataset(AppSetting.ActivateConnection, CommandType.Text, querybank);
                ...
                query = "SELECT TOP (100) PERCENT YEAR(dbo.vw_Allin1_ReportNew_S.ProducedOn) AS Year_S, ISNULL(SUM(CASE WHEN isnull(SUBSTRING(dbo.vw_Allin1_ReportNew_S.cerpac_no, 1, 2), 0) = 'AR' THEN 1 END), 0) + ISNULL(SUM(CASE WHEN isnull(SUBSTRING(dbo.vw_Allin1_ReportNew_S.cerpac_no, 1, 2), 0)  = 'AO' THEN 1 END), 0) + ISNULL(SUM(CASE WHEN isnull(SUBSTRING(dbo.vw_Allin1_ReportNew_S.cerpac_no, 1, 2), 0) = 'AB' + ISNULL(SUM(CASE WHEN isnull(SUBSTRING(dbo.vw_Allin1_ReportNew_S.cerpac_no, 1, 2), 0) = 'CR' THEN 1 END), 0) AS Total, dbo.vw_Allin1_ReportNew_S.ZoneName, ISNULL(SUM(CASE WHEN isnull(SUBSTRING(dbo.vw_Allin1_ReportNew_S.cerpac_no, 1, 2), 0) = 'AR' THEN 1 END), 0) AS AR, ISNULL(SUM(CASE WHEN isnull(SUBSTRING(dbo.vw_Allin1_ReportNew_S.cerpac_no, 1, 2), 0) = 'AO' THEN 1 END), 0) AS AO, ISNULL(SUM(CASE WHEN isnull(SUBSTRING(dbo.vw_Allin1_ReportNew_S.cerpac_no, 1, 2), 0) = 'AB', ISNULL(SUM(CASE WHEN isnull(SUBSTRING(dbo.vw_Allin1_ReportNew_S.cerpac_no, 1, 2), 0) = 'CR' THEN 1 END), 0) AS CR FROM dbo.vw_Allin1_ReportNew_S where (Month(ProducedOn)='" + dpdMon.SelectedValue + "' and Year(ProducedOn)='" + dpdYear.SelectedValue + "'or Month(producedonDate_C) ='" + dpdMon.SelectedValue + "'  and Year(producedonDate_C) ='" + dpdYear.SelectedValue + "') and ZoneName='" + dpd_zone.SelectedItem.ToString() + "' and (IsProduced=1 or IsProduced_C=1) GROUP BY dbo.vw_Allin1_ReportNew_S.ZoneName, YEAR(dbo.vw_Allin1_ReportNew_S.ProducedOn) ORDER BY Year_S";
                ...
                string qry_wasted = "Select  Count(distinct(card_no)) as wastecards,  CONVERT(varchar(3), created_on, 0)  as Month,  year(created_on) as Year  From tbl_lamination_detail where lam_printedYN=1 and  lam_wastedYN=1  and  Month(created_on)='" + dpdMon.SelectedValue + "' and  year(created_on)='" + dpdYear.SelectedValue + "' group by CONVERT(varchar(3), created_on, 0) ,  year(created_on)";
