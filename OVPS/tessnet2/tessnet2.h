...
namespace tessnet2 
{
	...
	public ref class Tesseract
	{
		...
		void PutImage(System::Drawing::Bitmap^ bitmap, System::Drawing::Rectangle rect);
	public:
		Tesseract();
		~Tesseract();
		...
		int								Init			(String^ tessdata, String^ lang, bool numericMode);
		void							Clear			();
		...
		// if OcrDoneEvent is null the OCR is synchronous, else asynchronous and callback when thread finished
		List<Word ^>^					DoOCR				(System::Drawing::Bitmap^ bitmap, System::Drawing::Rectangle rect);
		// Threshold
		// DoOCR use built in thresholding
		// To check this threshold result use this method to get thresholded image
